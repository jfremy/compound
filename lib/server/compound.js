...
CompoundServer.prototype.injectMiddlewareAfter = function(anchor, route, middleware) {
    ...
    mod.lazyrouter();
    mod._router.stack.forEach(function (r, i) {
        if (r.handle === anchor || r.handle.name === anchor) {
            anchorPosition = i;
        }
    });
...
CompoundServer.prototype.injectMiddlewareAt = function(position, route, middleware) {
    ...
    // Since the middleware stack is a lot more opaque, we cannot just insert the mw in it.
    // We first add the middleware (it will go at the end of the stack)
    mod.use(route, middleware);

    // Then we move the generate stack endtry accordingly
    if (position <= 0) {
        mod._router.stack.unshift(mod._router.stack.pop());
    } else if (position && position <= mod._router.stack.length) {
        mod._router.stack.splice(position, 0, mod._router.stack.pop());
    } else {
